-- Dashboard KPI RPCs for fast, auth-scoped aggregate reads.
...
create or replace function public.upcoming_renewals(
...
as $$
  with scoped as (
    select
      ...
      s.next_expected_at::date as next_expected_at,
      coalesce(abs(s.last_amount), 0)::numeric as last_amount,
      case
        when s.cadence = 'weekly' then coalesce(abs(s.last_amount), 0) * (52.0 / 12.0)
        when s.cadence = 'monthly' then coalesce(abs(s.last_amount), 0)
        when s.cadence = 'quarterly' then coalesce(abs(s.last_amount), 0) / 3.0
        when s.cadence = 'yearly' then coalesce(abs(s.last_amount), 0) / 12.0
        else coalesce(abs(s.last_amount), 0)
      end::numeric as monthly_equivalent,
      (s.next_expected_at::date - timezone('utc', now())::date)::int as days_until
    from public.subscriptions s
    where s.user_id = auth.uid()
      and s.is_active = true
      and s.is_false_positive = false
      and s.classification = 'subscription'
      and s.next_expected_at is not null
      and s.next_expected_at::date >= timezone('utc', now())::date
      and s.next_expected_at::date <= timezone('utc', now())::date + greatest(1, least(coalesce(lookahead_days, 14), 90))
  )
  select
    subscription_id,
    merchant_normalized,
    cadence,
    next_expected_at,
    round(last_amount, 2) as last_amount,
    round(monthly_equivalent, 2) as monthly_equivalent,
    days_until
  from scoped
  order by next_expected_at asc, merchant_normalized asc;
$$;

revoke all on function public.upcoming_renewals(integer) from public;
