-- Fix seed_user_categories to avoid referencing a non-existent named constraint.
-- Keep the auth.uid() security fix and preserve duplicate avoidance explicitly.
...
begin
  ...
  begin
    ...
    loop
      ...
      where not exists (
        select 1
        from public.transaction_category_rules_v1 existing
        where existing.user_id = v_user_id
          and existing.rule_type = v_rule_type
          and lower(existing.merchant_pattern) = lower(v_pattern)
          and existing.account_id is null
          and existing.min_amount is null
          and existing.max_amount is null
