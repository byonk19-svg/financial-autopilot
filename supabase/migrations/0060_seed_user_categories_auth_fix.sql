-- Harden seed_user_categories so authenticated callers cannot target another user's account.
-- Keep the existing signature for compatibility, but ignore the caller-supplied p_user_id.
