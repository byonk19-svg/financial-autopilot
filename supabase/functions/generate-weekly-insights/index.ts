...
const FUNCTION_NAME = "generate-weekly-insights";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (accountError) {
      return json(req, { error: "Could not resolve users." }, 500);
    }
    ...
    console.error(JSON.stringify({
      function: FUNCTION_NAME,
      action: "run_weekly_generation",
      message: details.message,
      stack: details.stack,
    }));
    return json(req, { error: "Weekly insight generation failed." }, 500);
  }
});
