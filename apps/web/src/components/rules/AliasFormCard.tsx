  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl border border-border bg-card p-5 shadow-sm">
      ...
      <div className="mt-3 space-y-3">
        <input value={form.pattern} onChange={(e) => onSetForm((c) => ({ ...c, pattern: e.target.value }))} placeholder='Match text (example: "comcast xfinity houston")' className="w-full rounded-lg border border-input bg-card px-3 py-2 text-sm text-foreground outline-none ring-ring focus:border-primary focus:ring-2" />
        <input value={form.normalized} onChange={(e) => onSetForm((c) => ({ ...c, normalized: e.target.value }))} placeholder='Rename to (example: "COMCAST")' className="w-full rounded-lg border border-input bg-card px-3 py-2 text-sm text-foreground outline-none ring-ring focus:border-primary focus:ring-2" />
